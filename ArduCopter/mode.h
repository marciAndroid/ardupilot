...
public:
    ...
    void set_target(const float roll_rate_rads, const float pitch_rate_rads, const float yaw_rate_rads, const float climb_rate_cms);

protected:

    const char *name() const override { return "RPI_CNTRL"; }
...
public:
    ...
    void set_target_attitude(const Quaternion &q, float climb_rate_cms, bool use_yaw_rate, float yaw_rate_rads);
...
private:
    GuidedNoGpsMode guided_nogps_mode = Guided_NoGPS_TakeOff;
